...
def run(document: papis.document.Document,
    ...
    # if neither files nor notes were deleted -> delte whole document
    if (filepath or notespath) is not None:
        if git:
...
@click.option(
    "-n", "--notes", "_notes",
    help="Remove the notes files from a document instead of the whole folder",
    is_flag=True,
    default=False)
@click.option(
    "-f", "--force",
